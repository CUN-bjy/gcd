...
def test_kmeans_semi_sup(merge_test_loader, args, K=None):
    ...
    for min_sample_num in range(3, 15):
        for eps_delta in range(1):
            eps = args.eps+eps_delta*0.02
            print("start fitting, min_sample_num: ", min_sample_num)
            dbscan = HDBSCAN( min_cluster_size=min_sample_num, n_jobs=100)
